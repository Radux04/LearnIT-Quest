# Allungare il Livello 1 — Alberi binari di ricerca (BST e Dijkstra)
...
| `scripts/graph/WeightedGraph.gd` | Il grafo pesato della Fase 5 e Dijkstra |
...
|---|---|---|
| Più o meno chiavi da posizionare | `Phase1.gd` | `VALUES_TO_PLACE` |
| Una radice diversa | `Level.gd` | `ROOT_VALUE` |
| Più ricerche da svolgere | `Phase2.gd` | `PRESENT_KEYS`, `ABSENT_KEYS` |
...
**Tre vincoli su `VALUES_TO_PLACE`:**
...
func _challenge_predecessor() -> void:
	...
	level.set_hint("È il nodo che viene prima di %s nella visita Inorder." % fmt(reference))
	await pick_node(target, false, "Corretto: prima di %s viene %s." % [
...
`await place_nodes(valori, penalità)` · `await search_key(chiave)` (gestisce anche il valore assente) · `await traverse_tree(tipo, mostra_regola)` · `await pick_node(valore, evidenzia, messaggio, penalità)` · `await delete_node(valore)` · `await shortest_path_game(grafo, sorgente, destinazione)`
...
**Con quello che c'è già** — una fase sulle **rotazioni AVL** (l'albero «si raddrizza» dopo un inserimento sbilanciato) · far scegliere al giocatore **dove inserire per tenere l'albero bilanciato**, mostrando la profondità risultante · una fase a **eliminazione multipla** dove l'ordine delle cancellazioni cambia la forma finale.

**Con un model nuovo** — **heap**: la stessa vista ad albero, ma la regola è padre ≤ figli, e il gesto è il «sift-up» dopo l'inserimento · **trie** di prefissi, con i nodi etichettati da lettere · **alberi di ricerca su stringhe** invece che su numeri.

**Sul grafo della Fase 5** — **A\*** con un'euristica mostrata a schermo, per far vedere la differenza con Dijkstra · **albero di copertura minimo** (Prim o Kruskal): il gesto è scegliere ogni volta l'arco più economico che non chiude un ciclo · **cammino minimo con pesi negativi** per far vedere dove Dijkstra sbaglia.
