...
## Livello 4 — Metodologie di programmazione.
##
## Stessa impalcatura del Livello 3: un «palco» (Stage) che ogni fase riempie
## con il proprio strumento, HUD e cronometro in comune.

const PHASE_SCRIPTS: Array[String] = [
	"res://scripts/phases/lvl4/Phase1.gd",
	"res://scripts/phases/lvl4/Phase2.gd",
	"res://scripts/phases/lvl4/Phase3.gd",
	"res://scripts/phases/lvl4/Phase4.gd",
]

const PHASE_BANNERS: Array = [
	["FASE 1 — CLEAN CODE", "Indica i difetti: nomi, numeri magici, duplicazione.", Color(0.4, 0.85, 1.0)],
	["FASE 2 — PRINCIPI SOLID", "Trova la violazione, poi separa le responsabilità.", Color(0.4, 1.0, 0.7)],
	["FASE 3 — RISCRIVI IL CODICE", "Togli i difetti senza cambiare il comportamento.", Color(1.0, 0.82, 0.35)],
	["FASE 4 — SCRIVI IL CODICE", "Una classe, una schermata JavaFX, una mappatura persistente.", Color(0.75, 0.65, 1.0)],
]
...
var phase_node: Lvl4PhaseBase = null
...
func _ready() -> void:
	...
	restart_button.pressed.connect(func() -> void: GameManager.go_to_intro_4())
	...
	pause_menu.restart_requested.connect(func() -> void: GameManager.go_to_intro_4())
	$Overlays.add_child(pause_menu)

	GameManager.start_level(GameManager.LEVEL4_DURATION)
	_run_level()
...
func _run_level() -> void:
	for i in range(PHASE_SCRIPTS.size()):
		...
		var phase: Lvl4PhaseBase = script.new()
...
func toast(text: String, color: Color = Color.WHITE) -> void:
	...
	_toast_tween.tween_interval(3.4)
...
func _on_time_expired() -> void:
	...
	end_subtitle.text = "La revisione non è stata completata in tempo.\nProve superate: %d." % solved_count
	_show_end_screen()
...
func _show_victory() -> void:
	...
	Sfx.play("victory")
	end_title.text = "CODICE APPROVATO"
	end_title.add_theme_color_override("font_color", Color(0.75, 0.65, 1.0))
	end_subtitle.text = "Hai completato il Livello 4 con %s sul cronometro.\n\nHai riconosciuto i difetti del codice, applicato i principi SOLID,\nrifattorizzato senza cambiare il comportamento e scritto classi,\ninterfacce JavaFX e mappature persistenti.\n\nProve superate: %d." % [
...
func _style_hud() -> void:
	...
	phase_title.add_theme_color_override("font_color", Color(0.55, 0.85, 1.0))
	...
	hint_label.add_theme_color_override("font_color", Color(0.66, 0.76, 0.95))
	...
	banner_sub.add_theme_color_override("font_color", Color(0.82, 0.88, 1.0))
